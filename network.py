...
class Network:
    ...
    def generate_real_traffic(self, filename, sw_list=None):  # real pcap/pkl -> syn traffic
        self.traffic = traffic.Traffic()
    ...
    def process_ctrl_messages(self, instractions):
        ...
        for inst in instractions:
            (act, obj, cont) = inst
            if act == setting.INST_FIX:
                sw = self.switches[obj]
                sw.add_entry(cont, fix=True)
            elif act == setting.INST_ADD:
                sw = self.switches[obj]
                isof = sw.add_entry(cont)
                if ret is None:
                    ret = isof
                else:
                    ret += isof
                ...
                if obj == setting.INST_OBJ_ALL:
                    ...
                    sw = self.switches[cont]
                    ret = sw.fix_table+sw.flow_table  # CAUTION: return reference
                else:
